...
class HRpgGauntlets : Weapon replaces Gauntlets
{
	...
	Hold:
		...
		Goto Ready;
	AltFire:
		GAUN LMN 4 BRIGHT A_GauntletAttack(0, 1);
		GAUN K 6;
		GAUN J 6 A_Light0;
		GAUN A 12 A_ReFire;
		Goto Ready;
	...
	action void A_GauntletAttack (int power, int altfire)
	{
		int damage;
		double dist;
		...
		{
			damage = random[GauntletAtk](1, 4) * 2;
			dist = 4*DEFMELEERANGE;
			ang += random2[GauntletAtk]() * (2.8125 / 256);
			pufftype = "GauntletPuff2";
		}
...
class HRpgGauntletsPowered : HRpgGauntlets replaces GauntletsPowered
{
	...
	Hold:
		...
		Goto Ready;
	AltFire:
		GAUN DEF 4 BRIGHT A_GauntletFireBolt();
		GAUN C 6;
		GAUN B 6 A_Light0;
		GAUN A 12 A_ReFire;
		Goto Ready;
	...
	action void A_GauntletFireBolt ()
	{
		if (player == null)
		{
			return;
		...
		let rangle = random[GauntletAtk](-6, 6);
